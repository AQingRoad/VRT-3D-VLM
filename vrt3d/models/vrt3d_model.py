...
from .object_centric_geometry_decoder import ObjectCentricGeometryDecoder
...
class DepthAware3DPositionEncoder(nn.Module):
    """
    ...
    def __init__(self, embedding_size, temperature=10000, n_points=1):
        super(DepthAware3DPositionEncoder, self).__init__()
...
@dataclass
class VRT3DCausalLMOutputWithPast(Qwen2_5_VLCausalLMOutputWithPast):
    """
    扩展 Qwen2.5-VL 的输出，使其包含 VRT-3D 几何解码所需的额外视觉字段。
    """
    past_image_embeds: Optional[torch.Tensor] = None
    past_high_res_image_embeds: Optional[torch.Tensor] = None
    past_visual_pe: Optional[torch.Tensor] = None
    past_logit_mask: Optional[torch.Tensor] = None
    last_hidden_states : Optional[torch.Tensor] = None

@dataclass
class VRT3DGenerateEncoderDecoderOutput(GenerateEncoderDecoderOutput):
...
@dataclass
class VRT3DGenerateDecoderOnlyOutput(GenerateDecoderOnlyOutput):
    past_image_embeds: Optional[torch.FloatTensor] = None
    past_logit_mask: Optional[torch.BoolTensor] = None
    past_high_res_image_embeds: Optional[torch.FloatTensor] = None
    past_visual_pe: Optional[torch.FloatTensor] = None
...
_old_vit_init = Qwen2_5_VisionTransformerPretrainedModel.__init__

def _vit_init_with_pcd(self, config, *args, **kwargs):
    print(config)
    _old_vit_init(self, config, *args, **kwargs)
    # 2) 在 vision 里注册 3D PE 与 gate（参数会进 state_dict）
    if config.use_3d_box:
        self.pcd_pe = DepthAware3DPositionEncoder(
            embedding_size=config.hidden_size, temperature=10000, n_points=1
        )
        self.pcd_pe_gate = nn.Parameter(torch.tensor(0.0))
        self.pcd_scale = 50.0  # 归一化尺度（按你的数据改）
Qwen2_5_VisionTransformerPretrainedModel.__init__ = _vit_init_with_pcd

def custom_visual_forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor,da3_predictions) -> torch.Tensor:
    """
    Args:
    ...
    if da3_predictions is not None:
        with torch.no_grad():
            depth = da3_predictions.depth
            K = da3_predictions.intrinsics
            assert depth is not None and K is not None
            depth = torch.tensor(depth).to(hidden_states.device)
            K = torch.tensor(K).to(hidden_states.device)
            if depth.dim() == 4:
                depth = depth[:,0]
            depth = depth.to(hidden_states.device)
            B,H,W = depth.shape
            if K.dim() == 2 and K.shape[-1] ==4:
                fx,fy,cx,cy = K[:,0],K[:,1],K[:,2],K[:,3]
            else:
                K = K.to(hidden_states.device)
                fx,fy,cx,cy = K[:,0,0],K[:,1,1],K[:,0,2],K[:,1,2]
            u = (torch.arange(W, device=depth.device, dtype=torch.float32) + 0.5)[None, None, :]
            v = (torch.arange(H, device=depth.device, dtype=torch.float32) + 0.5)[None, :, None]
            Z = depth.float()
            X = (u - cx[:, None, None].float()) / fx[:, None, None].float() * Z
            Y = (v - cy[:, None, None].float()) / fy[:, None, None].float() * Z
            xyz = torch.stack([X, Y, Z], dim=1)  # [B,3,H,W]
            valid = (Z > 0) & torch.isfinite(Z)
            valid3 = valid[:, None, :, :].expand_as(xyz)
            xyz = torch.where(valid3, xyz, torch.zeros_like(xyz))
            for b in range(B):
                vb = valid[b]
                if vb.any():
                    mean_xyz = xyz[b, :, vb].mean(dim=1)  # [3]
                    xyz[b] = torch.where(valid3[b], xyz[b], mean_xyz[:, None, None])
            xyz_seq_list = []
            for b, (t, h, w) in enumerate(grid_thw.tolist()):
                h = int(h); w = int(w); t = int(t)

                xyz_hw = F.interpolate(
                    xyz[b:b+1], size=(h, w), mode="bilinear", align_corners=False
                )[0].permute(1, 2, 0).contiguous()  # [h,w,3]

                # 与 rot_pos_emb 一致的 block-permute 顺序
                hpos_ids = torch.arange(h, device=hidden_states.device).unsqueeze(1).expand(-1, w)
                ...
                wpos_ids = torch.arange(w, device=hidden_states.device).unsqueeze(0).expand(h, -1)
                ...
                xyz_tok = xyz_hw[hpos_ids, wpos_ids]  # [h*w,3]
                if t > 1:
                    xyz_tok = xyz_tok.repeat(t, 1)
                xyz_seq_list.append(xyz_tok)

            xyz_seq = torch.cat(xyz_seq_list, dim=0)  # [seq_len,3]
            assert xyz_seq.shape[0] == seq_len

            # hidden_states 已经 window_index shuffle 了，所以 xyz_seq 必须同步 shuffle
            xyz_seq = xyz_seq.reshape(seq_len // self.spatial_merge_unit, self.spatial_merge_unit, 3)
            xyz_seq = xyz_seq[window_index, :, :].reshape(seq_len, 3)

            # 归一化（逐轴中心化 + 缩放）
            xyz_seq = xyz_seq - xyz_seq.mean(dim=0, keepdim=True)
            xyz_seq = xyz_seq / float(self.pcd_scale)
            xyz_seq = torch.nan_to_num(xyz_seq, nan=0.0, posinf=0.0, neginf=0.0)
        pe3d = self.pcd_pe(xyz_seq.unsqueeze(0).to(dtype=hidden_states.dtype)).squeeze(0)  # [seq_len, hidden]
        hidden_states = hidden_states + torch.tanh(self.pcd_pe_gate) * pe3d
...
class VRT3DForConditionalGeneration(Qwen2_5_VLForConditionalGeneration):
    def __init__(self, config):
        ...
        #     self.vl_decoder = ObjectCentricGeometryDecoder(self.config.vl_decoder, self.dtype)
        # else:
        #     pass
        self.vl_decoder = ObjectCentricGeometryDecoder(self.config.vl_decoder, self.dtype)
    ...
    def forward(self, *args, is_main=True, da3_predictions=None,**kwargs,):
        if is_main:
            return self.forward_main(*args, **kwargs,da3_predictions=da3_predictions)
        else:
    ...
    def forward_main(
        ...
        if inputs_embeds is None:
            if pixel_values is not None:
                ...
                image_embeds, high_res_image_embeds, visual_pe = self.visual(pixel_values, grid_thw=image_grid_thw,da3_predictions=da3_predictions)
        ...
        if past_image_embeds is not None:
            ret = VRT3DCausalLMOutputWithPast(
                loss=loss,
                logits=logits,
                ...
                past_high_res_image_embeds=high_res_image_embeds,
                past_visual_pe=visual_pe,
            ...
            ret = VRT3DCausalLMOutputWithPast(
                ...
                attentions=outputs.attentions,
                rope_deltas=self.rope_deltas,
                past_image_embeds = image_prototypes.clone(),
                past_logit_mask = logit_mask.clone(),
                past_high_res_image_embeds=high_res_image_embeds.clone(),
                past_visual_pe = visual_pe,
                last_hidden_states=outputs[0]
            )
        return ret
    ...
    def vl_decode(
        ...
        if len(cu_object_vp_feat) > 0:
            ...
            bbox_output, score_output, box3d_output, mask_logits, mask_HWs = self.vl_decoder(cu_object_vp_feat, cu_low_res_feats, cu_high_res_feats, cu_visual_pe, cu_patch, obj_image_grid_thws, self.device)
            return {
                ...
                'pred_box3d': box3d_output,
            }
        else:
            true_value = False
            cu_object_vp_feat = [torch.zeros((1, self.config.hidden_size), device=self.device, dtype=self.dtype)]
            cu_low_res_feats = torch.zeros((1, self.config.hidden_size), device=self.device, dtype=self.dtype)
            cu_high_res_feats = torch.zeros(
                (1 * (self.config.vision_config.spatial_merge_size ** 2), self.config.vision_config.hidden_size),
                device=self.device, dtype=self.dtype
            )
            cu_visual_pe = (
                torch.zeros((1 * (self.config.vision_config.spatial_merge_size ** 2),
            ...
            cu_patch = torch.tensor([0, self.config.vision_config.spatial_merge_size ** 2],
                                    device=self.device, dtype=torch.int32)
            obj_image_grid_thws = torch.tensor([[1, 2, 2]], device=self.device, dtype=torch.int64)

            bbox_output, score_output, box3d_output, mask_logits, mask_HWs = self.vl_decoder(cu_object_vp_feat, cu_low_res_feats, cu_high_res_feats, cu_visual_pe, cu_patch, obj_image_grid_thws, self.device)
            empty_boxes = bbox_output[:0]
            empty_score = score_output[:0]
            empty_box3d = box3d_output[:0] if isinstance(box3d_output, torch.Tensor) else torch.zeros((0, 8), device=self.device, dtype=self.dtype)

            if isinstance(mask_logits, torch.Tensor):
                empty_mask = mask_logits[:0]
            else:
                empty_mask = bbox_output.sum() * 0.0  # 占位且挂图
            empty_hw = (mask_HWs[0][:0], mask_HWs[1][:0]) if isinstance(mask_HWs, tuple) and len(mask_HWs) == 2 else ()

            return {
                "pred_boxes": empty_boxes,
                "pred_score": empty_score,
                "pred_mask": empty_mask,
                "pred_mask_valid_hw": empty_hw,
                "sample_idx": [],
                "pred_box3d": empty_box3d,
            }
        # cu_object_vp_feat = sum(object_vp_feats, [])
        ...
        # bbox_output, score_output,bbox_output_3d, mask_logits, mask_HWs = self.vl_decoder(cu_object_vp_feat, cu_low_res_feats, cu_high_res_feats, cu_visual_pe, cu_patch, obj_image_grid_thws, self.device)
    ...
    def generate(
        ...
        **kwargs,
        ...
        result = self._sample_vision_token(
            ...
            streamer=streamer,
            da3_predictions=da3_predictions,
            **model_kwargs,
        )
    ...
    def _sample_vision_token(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            if is_prefill:
                outputs = self(**model_inputs, return_dict=True,da3_predictions=da3_predictions)
                is_prefill = False
            else:
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                return VRT3DGenerateEncoderDecoderOutput(
                    sequences=input_ids,
                    scores=scores,
                    logits=raw_logits,
                ...
                return VRT3DGenerateDecoderOnlyOutput(
                    sequences=input_ids,
                    scores=scores,
                    logits=raw_logits,
...
PositionEmbeddingSine3D = DepthAware3DPositionEncoder
CustomQwen2_5_VLCausalLMOutputWithPast = VRT3DCausalLMOutputWithPast
CustomGenerateEncoderDecoderOutput = VRT3DGenerateEncoderDecoderOutput
CustomGenerateDecoderOnlyOutput = VRT3DGenerateDecoderOnlyOutput
PaDTForConditionalGeneration = VRT3DForConditionalGeneration

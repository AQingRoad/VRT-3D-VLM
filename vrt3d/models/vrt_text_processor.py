...
class VRTTextProcessor(object):
    """Wrap the base processor with trajectory tokens and dynamic VRT tokens."""
    ...
    def __init__(self, processing_class, spatial_merge_size=2):
        ...
        self._add_traj_tokens()
    def _add_traj_tokens(self):
        current_vocab = self.processing_class.tokenizer.get_vocab()
        tokens_to_add = [t for t in self.traj_tokens if t not in current_vocab]
        if tokens_to_add:
            self.processing_class.tokenizer.add_tokens(
                [AddedToken(t, lstrip=False, rstrip=False, special=True, normalized=False) 
                 for t in tokens_to_add]
            )
            self.model_embed_token_size = len(current_vocab) + len(tokens_to_add)
...
def parse_vrt_into_completion(processor, completion_ids, hidden_states, need_thinking_mask=None, image_prototype=None, image_grid_thw=None):
...
VisonTextProcessingClass = VRTTextProcessor
parseVRTintoCompletion = parse_vrt_into_completion

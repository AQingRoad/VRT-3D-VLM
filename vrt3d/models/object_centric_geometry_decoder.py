...
class GeometryDecoderFlashAttention2(nn.Module):
...
OBJECT_CENTRIC_DECODER_ATTENTION_CLASSES = {
    "flash_attention_2": GeometryDecoderFlashAttention2,
}


class ObjectCentricDecoderBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa", update_memory: bool = False) -> None:
        ...
        self.self_attn = OBJECT_CENTRIC_DECODER_ATTENTION_CLASSES[attn_implementation](
            config['hidden_size'], num_heads=config['num_heads']
        )
        self.cross_attn_query_to_image = OBJECT_CENTRIC_DECODER_ATTENTION_CLASSES[attn_implementation](
            config['hidden_size'], num_heads=config['num_heads']
        )
        ...
        if update_memory:
            self.cross_attn_image_to_query = OBJECT_CENTRIC_DECODER_ATTENTION_CLASSES[attn_implementation](
                config['hidden_size'], num_heads=config['num_heads']
            )
...
class ObjectCentricGeometryDecoder(nn.Module):
    """Decode VRT-aligned hidden states into continuous 2D/3D geometry."""
    ...
    def __init__(self, config, dtype):
        ...
        if config['use_3d_box']:
            self.addition_tokens = 4
        else:
            self.addition_tokens = 3
        self.bbox_score_mask_tokens = nn.Embedding(self.addition_tokens, config['hidden_size'])
        ...
        self.low_res_transformer = ObjectCentricDecoderBlock(config, config['attn_implementation'], update_memory=True)
        self.high_res_transformer1 = ObjectCentricDecoderBlock(config, config['attn_implementation'], update_memory=True)
        self.high_res_transformer2 = ObjectCentricDecoderBlock(config, config['attn_implementation'], update_memory=True)
        ...
        if config['use_3d_box']:
            self.bbox_3d_prediction = nn.Sequential(
                nn.Linear(config['hidden_size'], config['hidden_size']),
                nn.GELU(),
                nn.Linear(config['hidden_size'], config['hidden_size']),
                nn.GELU(),
            ...
            self.register_buffer(
                "box3d_center_scale",
                torch.tensor([50.0, 50.0, 5.0]).view(1, 3),
                persistent=False
            )
            self.register_buffer(
                "box3d_size_scale",
                torch.tensor([10.0, 10.0, 10.0]).view(1, 3),
                persistent=False
            )
    def _decode_box3d(self, raw3d: torch.Tensor) -> torch.Tensor:
        # raw3d: (N, 8)
        center = torch.tanh(raw3d[:, 0:3]) * self.box3d_center_scale.to(dtype=raw3d.dtype)
        size   = F.softplus(raw3d[:, 3:6]) * self.box3d_size_scale.to(dtype=raw3d.dtype)

        yaw = raw3d[:, 6:8]
        yaw = yaw / (yaw.norm(dim=-1, keepdim=True) + 1e-6)  # 投影回单位圆

        return torch.cat([center, size, yaw], dim=-1)
    ...
    def forward(self, object_vp_feat, cu_low_res_feats, cu_high_res_feats, visual_pe, cu_patch, obj_image_grid_thws, device):
        ...
        cu_query = []
        num_additional_token = self.addition_tokens  # box token, score token, mask token
        ...
        for obj_idx, obj_num in enumerate(vp_num_into_obj):
            ...
            acc_obj_num += obj_num
        # print(f"acc_obj_num {acc_obj_num}")
        ...
        bbox_score_mask_tokens = output_in_batch[:, :self.addition_tokens]
        
        if self.config['use_3d_box']:
            box2d_tok = bbox_score_mask_tokens[:, 0]
            box3d_tok = bbox_score_mask_tokens[:, 1]
            score_tok = bbox_score_mask_tokens[:, 2]
            mask_tok  = bbox_score_mask_tokens[:, 3]
            bbox_output = self.bbox_prediction(box2d_tok)
            score_output = self.score_prediction(score_tok)
            raw3d = self.bbox_3d_prediction(box3d_tok)
            box3d_output = self._decode_box3d(raw3d)
        else:
            # print(f"vl_decode cfffff")
            box2d_tok = bbox_score_mask_tokens[:, 0]
            score_tok = bbox_score_mask_tokens[:, 1]
            mask_tok  = bbox_score_mask_tokens[:, 2]
            bbox_output = self.bbox_prediction(box2d_tok)
            score_output = self.score_prediction(score_tok)
            # box3d_output = self.bbox_3d_prediction(box3d_tok)
            box3d_output = None

        if self.use_mask_loss is False:
            return bbox_output, score_output, box3d_output, ()

        mask_output = self.mask_output_mlp(mask_tok)
        ...
        return bbox_output, score_output, box3d_output, masks_per_object, (Hs, Ws)


# Backward-compatible aliases for older imports/checkpoints.
PaDTDecoderFlashAttention2 = GeometryDecoderFlashAttention2
PaDTDecoder_VISION_ATTENTION_CLASSES = OBJECT_CENTRIC_DECODER_ATTENTION_CLASSES
PaDTDecoderBlock = ObjectCentricDecoderBlock
PaDTDecoder = ObjectCentricGeometryDecoder
